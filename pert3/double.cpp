...
void CF(Node *&head, int &jumlahRoster)
{
    ...
    nodeBaru->prev = nullptr;        // Prev berisi null karna di awal dan bukan circular
    nodeBaru->next = head;           // Dalam menambahkan di awal, node baru akan mengambil alamat di head, menghubungkan Node baru ke linked list
...
{
    if (head == nullptr) // Error handling jika linked list masih kosong beralih ke CF()
    {
        CF(head, jumlahRoster);
        return;
    }
    ...
    nodeBaru->prev = temp;           // Menghubungkan prev ke node sebelumnya
    nodeBaru->next = nullptr;        // Alamat next Node baru kosong karna akan ditaruh di akhir
    temp->next = nodeBaru;           // alamat node baru di hubungkan ke node terujung yang kosong tadi
...
void CP(Node *&head, int &jumlahRoster)
{
    ...
    if (0 < input and input < jumlahRoster + 2) // Memastikan input berada dalam batas yang valid
    {
        ...
        else if (input == jumlahRoster + 1) // Jika input adalah akhir dari list beralih ke CL()
        {
            CL(head, jumlahRoster);
            return;
        }
        ...
        nodeBaru->prev = temp;           // Menghubungkan prev Node baru ke Node sebelumnya dari urutan yang dipilih
        nodeBaru->next = temp->next;     // next Node baru menunjuk ke node setelahnya (Node urutan yang dipilih)
...
void DF(Node *&head, int &jumlahRoster)
{
    ...
    if (head != nullptr) // Memutuskan hubungan ke node sebelumnya yang akan di hapus jika bukan node terakhir di list
    {
        head->prev = nullptr;
    }
...
{
    if (head->next == nullptr) // Jika hanya ada satu node di linked list beralih ke DF
    {
...
void DP(Node *&head, int &jumlahRoster)
{
    ...
    if (0 < input and input < jumlahRoster + 1) // Memastikan input berada dalam rentang yang valid
    {
        ...
        else if (input == jumlahRoster + 1) // Jika input urutan terakhir, hapus node akhir
        {
        ...
        (cur->next)->prev = temp; // Menghubungkan node setelah yang ingin dihapus ke sebelumnya
        delete cur;
        jumlahRoster = jumlahRoster - 1; // Mengurangi jumlah roster
        cout << "\n Telah dihapus! \n";  // Memberi tahu user bahwa node telah dihapus
    }
