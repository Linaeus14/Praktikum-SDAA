...
// Node double circular linked list, hanya memiliki data dan pointer node next;
struct Node
{
    Node *prev;
    Data data;
...
void Read(Node *head, int jumlahRoster);
void Update(Node *&head);
void Delete(Node *&head, Node *&tail, int &jumlahRoster, string menu);

// Prosedur Manipulasi double circular Linked List
void CF(Node *&head, Node *&tail, int &jumlahRoster); // Tambah Awal
void CL(Node *&head, Node *&tail, int &jumlahRoster); // Tambah Akhir
void CP(Node *&head, Node *&tail, int &jumlahRoster); // Tambah Urutan Tertentu
void DF(Node *&head, Node *&tail, int &jumlahRoster); // Hapus Awal
void DL(Node *&head, Node *&tail, int &jumlahRoster); // Hapus Akhir
void DP(Node *&head, Node *&tail, int &jumlahRoster); // Hapus Urutan Tertentu
...
int main()
{
    ...
    Node *tail = nullptr;        // head yang akhiran awalan dari linked list
...
Node *CTemp(Node *&head, Node *&tail) // Penambahan untuk awal dan akhir double circular bisa dibilang sama, hanya berbeda pada deklarasi tail dan head di akhir, maka dibuat function Create temporary
{
    Node *nodeBaru = new Node;
    nodeBaru->data = inputData().data;

    if (tail == nullptr) // Jika link list masih kosong, head dan tail akan sama saat ada 1 node saja
    {
        nodeBaru->prev = nodeBaru; // Dalam circular linked list, saat ada 1 node saja maka akan merefrensi diri sendiri (berlaku untuk prev dan next)
        nodeBaru->next = nodeBaru;
        tail = nodeBaru;
    }
    else
    {
        nodeBaru->prev = tail; // Mengambil tail untuk digubungkan ke prev
...
{
    head = CTemp(head, tail);  // Head akan menjadi alamat dari Node baru ditambah
    (head->next)->prev = head; // Menghubungkan Node kedua ke head di bagian prev
    tail->next = head;         // Menghubungkan tail ke head baru di bagain next
...
void CL(Node *&head, Node *&tail, int &jumlahRoster)
{
    ...
    tail = CTemp(head, tail);  // tail akan menjadi alamat dari Node baru yang ditambah
    (tail->prev)->next = tail; // Menghubungkan Node akhir sebelumnya ke tail baru
    head->prev = tail;         // Menghubungkan head ke tail di bagian prev
...
void CP(Node *&head, Node *&tail, int &jumlahRoster)
{
    ...
    if (0 < input and input < jumlahRoster + 2) // Memastikan input berada dalam batas yang valid
    {
        ...
        nodeBaru->next = temp->next;     // Node baru menunjuk ke node setelahnya (node di urutan input)
        nodeBaru->prev = temp;           // Merujuk ke node input sebagai node sebelumnya
        (temp->next)->prev = nodeBaru;   // Merujuk ke node setelah node inputan untuk di  hubungkan ke node baru melalui prev
...
void Read(Node *head, int jumlahRoster)
{
    ...
    for (int x = 0; x < jumlahRoster; x++) // Loop melalui seluruh node dalam linked list untuk menampilkan data
    {
        cout << endl
             << no
             ...
             << "   Alamat    : " << head << endl                                                                                    // Menampilkan alamat data
             << "   Prev      : " << head->prev << endl                                                                              // Menampilkan alamat sebelumnya
             << "   Next      : " << head->next << endl;                                                                             // Menampilkan alamat selanjutnya
...
void Update(Node *&head)
{
    ...
    while (temp->next != head) // Loop melalui linked list
    {
...
void Delete(Node *&head, Node *&tail, int &jumlahRoster, string menu)
{
    ...
    while (loop == true) // Loop yang terus berjalan hingga pilihan valid dipilih
    {
        ...
        {
        case 1:
            DF(head, tail, jumlahRoster); // Hapus node pertama dalam linked list
            loop = false;                 // Keluar dari loop setelah operasi berhasil
            break;

        case 2:
            DL(head, tail, jumlahRoster); // Hapus node terakhir dalam linked list
            loop = false;                 // Keluar dari loop setelah operasi berhasil
            break;

        case 3:
            DP(head, tail, jumlahRoster); // Hapus node di posisi tertentu dalam linked list
            loop = false;                 // Keluar dari loop setelah operasi berhasil
            break;
...
void DF(Node *&head, Node *&tail, int &jumlahRoster)
{
    ...
    else
    {
        ...
        head->prev = tail;
        tail->next = head; // Merefrence tail ke head yang baru
    }
...
{
    if (head->next == head) // Jika hanya ada satu node di linked list beralih ke deletefirst
    {
        DF(head, tail, jumlahRoster);
        return; // Kembali dari fungsi
    }

    Node *cur = tail;                // Menyimpan alamat node sebelum di hapus hubungan
    tail = tail->prev;               // Memundurkan tail 1 node
    tail->next = head;               // Menghapus node terakhir dengan mengatur next menjadi head
    head->prev = tail;               // mengisi prev head dengan Node akhir yang baru
    delete cur;                      // Menghapus data di alamat node sebelumnya
...
{
    int input;
    Read(head, jumlahRoster); // Menampilkan semua karakter sebelum menghapus
    ...
    if (0 < input and input < jumlahRoster + 1) // Memastikan input berada dalam rentang yang valid
    {
        ...
        Node *temp = head, *temp2, *cur;    // Pointer Node sementara untuk traversing dan Pointer Node yang ingin dihapus
        for (int x = 0; x < input - 2; x++) // Mencari node sebelum posisi yang ingin dihapus
        {
            temp = temp->next; // Pindah ke node berikutnya
        }
        cur = temp->next;  // Node yang ingin dihapus
        temp2 = cur->next; // node kanan dari yang ingin dihapus (temp node kirinya)

        // Penghubungan antar node kiri dan kanan
        temp2->prev = temp; // hubungkan prev node kanan ke node kiri
        temp->next = temp2; // Menghubungkan next node kiri ke node setelah node yang ingin dihapus (kanan)

        delete cur;                      // Hapus memori
        jumlahRoster = jumlahRoster - 1; // Mengurangi jumlah roster
        cout << "\n Telah dihapus! \n";  // Memberi tahu user bahwa node telah dihapus
    }
