...
// Node single circular linked list, hanya memiliki data dan pointer node next;
struct Node
{
    Data data;
    Node *next;
...
// Prosedur Manipulasi Single Circular Linked List
...
void DP(Node *&head, Node *&tail, int &jumlahRoster); // Hapus tertentu
...
void CF(Node *&head, Node *&tail, int &jumlahRoster)
{
    ...
    {
        nodeBaru->next = nodeBaru; // Dalam circular linked list, saat ada 1 node saja maka akan merefrensi diri sendiri (berlaku untuk prev dan next)
        tail = nodeBaru;
    }
    else
    {
    ...
    tail->next = head; // Menghubungkan kembali tail ke head
...
void CL(Node *&head, Node *&tail, int &jumlahRoster)
{
    ...
    Node *nodeBaru = new Node;
    nodeBaru->data = inputData().data;
    nodeBaru->next = head; // Alamat next Node baru mengarah ke head karna akan ditaruh di akhir sehingga menjadi circular

    tail->next = nodeBaru;           // alamat node baru di tambahkan ke alamat node terujung yang kosong melalui tail, menghubungkannya
    tail = nodeBaru;                 // Mengubah tail ke node terujung, yaitu node baru yang tadi
...
void DL(Node *&head, Node *&tail, int &jumlahRoster)
{
    ...
    while (temp->next->next != head) // Mencari node sebelum node terakhir
    {
        temp = temp->next; // Pindah ke node berikutnya
    }
    Node *cur = temp->next;          // Menyimpan alamat node sebelum di hapus hubungan
    temp->next = head;               // Menghapus node terakhir dengan mengatur next menjadi head
    tail = temp;                     // mengisi tail dengan Node akhir yang baru
    delete cur;                      // Menghapus data di alamat node sebelumnya
